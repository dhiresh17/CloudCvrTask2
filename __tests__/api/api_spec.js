...
describe('BADGES/Tags', function () {
    it ('Check Data types of GET BADGES TAGS call', function (done) {
        return frisby
          .get('https://api.stackexchange.com/2.2/badges/tags?order=asc&sort=name&site=stackoverflow')
          .expect('status', 200)
          .expect('jsonTypes', 'items.*', { // Assert *each* object in 'items' array
          'badge_type': Joi.string().required(),
          'award_count': Joi.number().required(),
...
describe('BADGES/Tags', function () {
    it('Check Tag names are sorted in ascending order', function (done) {
        return frisby
            .get('https://api.stackexchange.com/2.2/badges/tags?order=asc&sort=name&site=stackoverflow')
            .expect('status', 200)
            .then((res) => {
...
describe('BADGES/Tags', function () {
    it('Check Tag names are sorted in descending order', function (done) {
        return frisby
            .get('https://api.stackexchange.com/2.2/badges/tags?order=desc&sort=name&site=stackoverflow')
            .expect('status', 200)
            .then(function(res) {
                //console.log(res.body.items);
                const data = JSON.parse(res['body']);
...
describe('BADGES/tags', function () {
    it('Check rank filter is correct', function (done) {
        return frisby
            .get('https://api.stackexchange.com/2.2/badges/tags?order=desc&min=bronze&max=bronze&sort=rank&site=stackoverflow')
            .expect('status', 200)
            .then(function(res) {
                //console.log(res.body.items);
                const data = JSON.parse(res['body']);
                const items = data.items;
                const originalRankNames = items.map((item) => item.rank);
                expect(originalRankNames.every((val, i, originalRankNames) => val === 'bronze')).toBe(true)
            })
            .done(done);
    });
});
